...
var autoWeighoff = func
{
	if (getprop("/gear/gear[0]/wow"))
	{
	   ...
	   print("L30: Auto weigh off from " ~ (-lift) ~ " lb heavy to neutral.");
...
var init_all = func(reinit=0)
{
    ...
    {
        # Livery support.
        aircraft.livery.init("Aircraft/L30/Models/Liveries");
        ...
        props.globals.getNode(gascell ~ "[0]", 1).alias(props.globals.getNode(fdm ~ "[0]/valve_open"));
        props.globals.getNode(gascell ~ "[1]", 1).alias(props.globals.getNode(fdm ~ "[1]/valve_open"));
        props.globals.getNode(gascell ~ "[2]", 1).alias(props.globals.getNode(fdm ~ "[2]/valve_open"));
        props.globals.getNode(gascell ~ "[3]", 1).alias(props.globals.getNode(fdm ~ "[3]/valve_open"));
        props.globals.getNode(gascell ~ "[4]", 1).alias(props.globals.getNode(fdm ~ "[4]/valve_open"));
        props.globals.getNode(gascell ~ "[5]", 1).alias(props.globals.getNode(fdm ~ "[5]/valve_open"));
        props.globals.getNode(gascell ~ "[6]", 1).alias(props.globals.getNode(fdm ~ "[6]/valve_open"));
        props.globals.getNode(gascell ~ "[7]", 1).alias(props.globals.getNode(fdm ~ "[7]/valve_open"));
        props.globals.getNode(gascell ~ "[8]", 1).alias(props.globals.getNode(fdm ~ "[8]/valve_open"));
        props.globals.getNode(gascell ~ "[9]", 1).alias(props.globals.getNode(fdm ~ "[9]/valve_open"));
        props.globals.getNode(gascell ~ "[10]", 1).alias(props.globals.getNode(fdm ~ "[10]/valve_open"));
        props.globals.getNode(gascell ~ "[11]", 1).alias(props.globals.getNode(fdm ~ "[11]/valve_open"));
        props.globals.getNode(gascell ~ "[12]", 1).alias(props.globals.getNode(fdm ~ "[12]/valve_open"));
        props.globals.getNode(gascell ~ "[13]", 1).alias(props.globals.getNode(fdm ~ "[13]/valve_open"));
        props.globals.getNode(gascell ~ "[14]", 1).alias(props.globals.getNode(fdm ~ "[14]/valve_open"));
        props.globals.getNode(gascell ~ "[15]", 1).alias(props.globals.getNode(fdm ~ "[15]/valve_open"));
        props.globals.getNode(gascell ~ "[16]", 1).alias(props.globals.getNode(fdm ~ "[16]/valve_open"));
        setprop("controls/gas/overpressure[0]", 0);
        setprop("controls/gas/overpressure[1]", 0);
        setprop("controls/gas/overpressure[2]", 0);
        setprop("controls/gas/overpressure[3]", 0);
        setprop("controls/gas/overpressure[4]", 0);
        setprop("controls/gas/overpressure[5]", 0);
        setprop("controls/gas/overpressure[6]", 0);
        setprop("controls/gas/overpressure[7]", 0);
        setprop("controls/gas/overpressure[8]", 0);
        setprop("controls/gas/overpressure[9]", 0);
        setprop("controls/gas/overpressure[10]", 0);
        setprop("controls/gas/overpressure[11]", 0);
        setprop("controls/gas/overpressure[12]", 0);
        setprop("controls/gas/overpressure[13]", 0);
        setprop("controls/gas/overpressure[14]", 0);
        setprop("controls/gas/overpressure[15]", 0);
        setprop("controls/gas/overpressure[16]", 0);
            settimer(func {
    ...
    spy_timer = maketimer(0.1, updateSpySight);
...
setlistener("/sim/signals/fdm-initialized", func {
    ...
    setprop("/controls/engines/engine[5]/opstatereq", 0);
    setprop("/controls/engines/engine[5]/opstateresp", 0);
    ...
    print("L30 Systems ... Check");

    setlistener("/sim/signals/click", func {
        var click_pos = geo.click_position();
        if (__kbd.ctrl.getBoolValue()) {
            L30.ground_crew.place_ground_crew(click_pos,
                                                         nil,
                                                         click_pos.alt());
        }
    });
...
io.load_nasal(getprop("/sim/aircraft-dir") ~ "/Systems/scenario-network.nas", "L30");
...
var releaseBallast = func(location, amount)
{
    ...
    if (location == FORE_BALLAST) {
        ...
        printlog("warn",
                 "L30.releaseBallast(" ~ location ~ ", " ~ amount ~
...
var refillQuickReleaseBallast = func()
{
    ...
    foreach(var tb; trim_ballast_p)
    {
		...
		if (tmp < 0)
		{
			last = last + tmp;
			tmp = 0;
			gui.popupTip("Trim Ballast empty");
		}
...
forindex(var i; aft_ballast_toggles_p)
{
    setlistener(aft_ballast_toggles_p[i], func (n) {
        ...
        interpolate(aft_ballast_p[n.getIndex() - size(fore_ballast_toggles_p)], 0.0, 1.0);
    }, 0, 0);
}
...
var setForwardGasValves = func (v) {
    setprop(gascell ~ "[16]", v);
    setprop(gascell ~ "[15]", v);
    setprop(gascell ~ "[14]", v);
    setprop(gascell ~ "[13]", v);
    setprop(gascell ~ "[12]", v);
...
{
	var lift = 5905.51181102 - (getprop("/controls/flight/elevator-trim") * 1581);
...
var GetTankState = func(eng)
{
   ...
   if (eng == 5)
   {
      if (getprop("/consumables/fuel/tank[10]/empty") and getprop("/consumables/fuel/tank[11]/empty"))
      {
         return 0;
      }
      else
...
var updateMixture = func()
{
   ...
   {
      if (getprop("fdm/jsbsim/buoyant_forces/gas-cell[" ~ i ~ "]/volume-ft3") >= (getprop("fdm/jsbsim/buoyant_forces/gas-cell[" ~ i ~ "]/max_volume-ft3") - 0.1))
	  {
         setprop("controls/gas/overpressure[" ~ i ~ "]", 1);
	  }
	  else
	  {
         setprop("controls/gas/overpressure[" ~ i ~ "]", 0);
	  }
   }
}
...
var checkView = func()
{
    ...
    if (getprop("/sim/current-view/name") == "Observer Car View")
    {
       if (!spy_timer.isRunning)
       {
	      var nOff = getprop("/sim/weapons/spyoff");
          setprop("/sim/current-view/y-offset-m", -11.25 + nOff);
       }
    }
}
...
var updateBombSight = func()
{
    ...
    var vorh = ((0.15912 / h) * ((math.sqrt((h * 2) / 9.81) * v) - 64)) / 2.0;
    setprop("/sim/weapons/sightoff", -vorh);
#    print("sight vorhalt " ~ vorh);
}


var lowerSpySight = func()
{
   var spyInc = getprop("/sim/weapons/spywinch");
   if (spy_timer.isRunning)
   {
      spyInc = 0;
      spy_timer.stop();
      gui.popupTip("Stop winching Observer car.");
   }
   else
   {
      spyInc = 0.5;
      spy_timer.start();
      gui.popupTip("Winching Observer car down.");
   }
   setprop("/sim/weapons/spywinch", spyInc);
}

var raiseSpySight = func()
{
   var spyInc = getprop("/sim/weapons/spywinch");
   if (spy_timer.isRunning)
   {
   ...
   else
   {
      spyInc = -0.5;
      spy_timer.start();
      gui.popupTip("Winching Observer car up.");
   }
   setprop("/sim/weapons/spywinch", spyInc);
}
 
var updateSpySight = func()
{
   var spyInc = getprop("/sim/weapons/spywinch");
   if (getprop("sim/model/door-positions/SDoor/position-norm") < 1)
   {
      gui.popupTip("open doors first.");
      setprop("/sim/weapons/spyoff", 0);
      spy_timer.stop();
      return;
   }
	var nOff = getprop("/sim/weapons/spyoff");
    ...
    if (nOff > 0)
    {
        nOff = 0;
        spy_timer.stop();
        gui.popupTip("Observer car is back.");
        setprop("/sim/weapons/spywinch", 0);
    }
    if (nOff < -1000)
    {
        nOff = -1000;
        spy_timer.stop();
        gui.popupTip("Observer car can't get lower.");
        setprop("/sim/weapons/spywinch", 0);
    }
    if (getprop("/sim/current-view/name") == "Observer Car View")
    {
    ...
    setprop("/sim/weapons/spyoff", nOff);
    setprop("/sim/weapons/spyscale", (0.324435 + (-nOff)) / 0.324435);
}
...
var dropBombsWork = func()
{
   ...
   if (getprop("/position/altitude-agl-ft") < 00)
   {
...
var impactBomb = func(n)
{
   ...
   if (solid)
   {
      ...
      geo.put_model("Aircraft/L30/Models/crater.xml", lat, lon, ele + 0.1, hea, 0, 0);
      geo.put_model("Aircraft/L30/Models/bombe.xml", lat, lon, ele + 0.25, hea, 0, 0);
...
var start_terrain_fire = func ( lat_deg, lon_deg, alt_m=0, ballisticMass_lb=1.2 ) {
  ...
  time_sec=75; 
  fp1="Aircraft/L30/Models/bombable/Fire-Particles/fire-particles-large.xml";
  put_remove_model(lat_deg:lat_deg, lon_deg:lon_deg, elev_m:alt_m, time_sec:time_sec, startSize_m: nil, endSize_m:nil, path:fp1 );
  time_sec1=300; 
  fp="Aircraft/L30/Models/bombable/Fire-Particles/smoke-particles-large.xml";
  ...
  {
    time_sec2=75; fp2="Aircraft/L30/Models/bombable/Fire-Particles/fire-particles-very-small.xml";  
    ...
    time_sec3=75; fp3="Aircraft/L30/Models/bombable/Fire-Particles/fire-particles-very-very-small.xml";  
    ...
    time_sec4=75; fp4="Aircraft/L30/Models/bombable/Fire-Particles/fire-particles-very-very-very-small.xml";  
...
var SmoothTransfer = {
    ...
    _loop_: func()
    {
        ...
        me.from.setValue(me.from.getValue() - a);
...
#################################################################
    create : func {
        ...
        w.set("default", 0);
        w.set("key", "esc");
        w.setBinding("nasal", "L30.dialog.destroy(); ");
        ...
        props.globals.initNode("sim/about/text",
             "Zeppelin L30 (LZ62) airship for FlightGear\n" ~
...
var weather_dialog = gui.Dialog.new("/sim/gui/dialogs/L30/weather/dialog", "Aircraft/L30/Dialogs/weather-report.xml");
var autopilot_dialog = gui.Dialog.new("sim/gui/dialogs/autopilot/dialog", "Aircraft/L30/Dialogs/autopilot.xml");
