...
#    print("Message from "~ sender.getNode("callsign").getValue() ~ " size: " ~ size(msg) ~ " Type: " ~ type);
    if (type == message_id["bomb_impact"][0])
    {
        ...
        geo.put_model("Aircraft/L30/Models/crater.xml", pos.lat(), pos.lon(), pos.alt() + 0.1, 0, 0, 0);
        geo.put_model("Aircraft/L30/Models/bombe.xml", pos.lat(), pos.lon(), pos.alt() + 0.25, 0, 0, 0);
        start_terrain_fire(pos.lat(), pos.lon(), pos.alt() + 0.25, 230);
    }
    if (type == message_id["place_ground_crew"][0])
    {
        L30.ground_crew.place_remote_ground_crew(sender.getPath(),
...
var listen_to = func (pilot) {
    if (pilot.getNode("sim/model/path") != nil and
        streq("Aircraft/L30/Models/L30.xml",
...
var when_disconnecting = func (pilot) {
    L30.ground_crew.remove_remote_ground_crew(pilot.getPath());
}
...
var remote_ground_crew = {
    ...
    init : func {
        me.model = {};
        me.model_path =
            me.find_model_path("L30/Models/GroundCrew/wire-party.xml");
    },
    ##################################################
